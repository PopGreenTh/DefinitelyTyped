...
declare namespace jasmine {
    interface JasmineStatic {
        new(options?: any): Jasmine;
        (options?: any): Jasmine;
    }

    interface Jasmine {
        randomizeTests(value: any);
        seed(value: any);
        showColors(value: any);
        addSpecFile(filePath: string);
        addReporter(reporter: any);
        configureDefaultReporter(options: any);
        addMatchers(matchers: any);
        loadSpecs();
        loadHelpers();
        loadConfigFile(filePath: string);
        loadConfig(config: any);
        addSpecFiles(files: string[]);
        onComplete(onCompleteCallback: (passed: boolean) => void);
        stopSpecOnExpectationFailure(value: any);
        execute(files: string[], filterString?: string);
    }
}

declare var Jasmine: jasmine.JasmineStatic;
